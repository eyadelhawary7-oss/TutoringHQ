...
4. **PR state comes from the PR, never from a branch SHA or a status note.** Two rules, both learned the expensive way on 28 July, each after the same mistake twice: (a) **a branch left at an old SHA after a squash merge is not an open PR** — squashing does not move or delete the branch, so its head matches forever while the work is long merged; check `merged`/`state` on the PR itself, or look for the squash commit in `git log origin/master`; (b) **check merge state against master, not against your own check-in log or session notes** — `git fetch origin && git log --oneline origin/master` first, every time. A check-in that says "unchanged across six checks" is describing its own memory, not the repository. Getting this wrong re-queues finished work and costs a turn each time.
5. Migrations are manual apply to production. Supabase Branching auto-applies to preview branches only, never to production on merge. This was tested on 2026-07-15: PR #159 merged as 80f82ba and the migration was still absent from the production catalog and from the production migration history 8 minutes later. Apply the migration by hand, confirm the columns exist in information_schema, then let the code deploy. Never merge and assume. An inference is not a finding: the earlier claim that Branching auto-applies on merge was a guess presented as an answer, and it caused a deploy that read columns which did not exist.
6. **Never cite a count you did not just run.** Every number that reaches a PR body, a ledger entry, a code comment or a message to Eyad — row counts, call-site counts, file counts, coverage fractions, percentages, "N of M" — must come from a command executed in that session, and per-part figures must sum to the total stated beside them. This is a *separate* failure from rule 2 and it is the more common one: rule 2 catches a wrong conclusion, this catches **a right conclusion resting on fabricated evidence**. Named on 5 August 2026 after an adversarial re-verify of twenty PRs found it in almost every one, in the same shape each time — `schedule/page.tsx` given as 8 call sites when `grep -c` says 9; `preferred_locale` "read in exactly four places" when it is 17 occurrences across 8 files and one of the four is a write; a `7.5/10 → 8.5/10` coverage claim that exists in no ledger file; a `≈4.2/5` headline whose own three sub-scores average 3.77. Earlier instances of the identical pattern: the "11 adopters" grep and the `OrdersPageClient` false negative. **A bucketed count presented as a precise one is this rule broken**, as is describing a check as "every X read by hand" when the list was read and the counts were estimated. If a figure cannot be re-derived on demand, do not state it as a figure — say what was actually checked.
