...
describe('the verification gate — DERIVED from the identity engine, not a constant', () => {
  const principal = {
  ...
  /**
   * A Supabase whose verification_records read fails the way an unapplied
   * migration actually fails: PostgREST 42P01, undefined table. This is the
   * error `isMissingRelation()` in verificationStore.ts exists to catch.
   */
  function noIdentityTablesSupabase() {
    const missing = {
      code: '42P01',
      message: 'relation "public.verification_records" does not exist',
    };
    const chain = {
      select: () => chain,
      eq: () => chain,
  ...
  /** Set real-looking VALIFY_* values so the guard stops short-circuiting. */
  function withRealValifyEnv<T>(fn: () => T): T {
    const keys = [
      'VALIFY_API_KEY',
      'VALIFY_BASE_URL',
      'VALIFY_FLOW_ID',
  ...
  it('refuses because the Valify credentials are placeholders — the state of the world today', async () => {
    // No VALIFY_* in the environment, which is what every deployment currently
    // has. The gate must reach this by ASKING the guard, not by being hardcoded.
    ...
    expect(r.cause).toBe('verification_provider_not_configured');
    expect(r.messageKey).toBe('collectionPayout.verification.providerNotConfigured');
    ...
    // in a politer voice.
    expect(r.blockedOn).toContain('VALIFY_');
  });

  it('refuses because the state has NO LIVE SOURCE once the credentials are real', async () => {
    // Proof the refusal is derived: with the same unmigrated database and only
    // the credentials changed, the CAUSE changes. A constant cannot do that.
    const r = await withRealValifyEnv(() =>
      resolvePrincipalVerification(noIdentityTablesSupabase(), principal),
    );
    expect(r.verified).toBe(false);
    if (r.verified) throw new Error('unreachable');
    ...
    expect(r.blockedOn).toContain('verification_records');
  });

  it('refuses a centre principal with no centerId rather than falling back to its userId', async () => {
    // Gating one subject on another subject's verification is how money reaches
    // the wrong person.
    const r = await resolvePrincipalVerification(unmigratedSupabase(), {
      ...principal,
      centerId: null,
    });
    expect(r.verified).toBe(false);
    if (r.verified) throw new Error('unreachable');
    expect(r.cause).toBe('verification_state_unreadable');
  });
  ...
  it('every refusal cause has copy in BOTH message files', async () => {
    // The gate can only emit keys under collectionPayout.verification.*; all of
    // them must resolve, or a refused owner sees a raw key.
    const en = enMessages.collectionPayout.verification as Record<string, string>;
    const ar = arMessages.collectionPayout.verification as Record<string, string>;
    for (const k of [
      'stateNotInSchema',
      ...
      expect(typeof en[k], `en missing ${k}`).toBe('string');
      expect(typeof ar[k], `ar missing ${k}`).toBe('string');
    }
  });
});
