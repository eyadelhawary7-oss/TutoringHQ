...
// signup is charged ONE month at the all-in per-month rate (monthly == quarterly
// per-month rate), never the ×3 quarterly figure, and that annual stays
// monthly × annualMultiplier. Guards the brief's invariant: "no new activity
// produces a three-month period or a times-three amount."
describe('center signup cadence — monthly is one month, never ×3', () => {
  for (const key of ['solo', 'nano', 'starter', 'pro', 'business', 'enterprise'] as const) {
    ...
    it(`${key}: monthly signup amount = all-in per-month rate (not quarterlyAllIn × 3)`, () => {
      const monthly = getPlanPrice(key, 'monthly');
      expect(monthly).toBe(plan.quarterlyAllIn);
