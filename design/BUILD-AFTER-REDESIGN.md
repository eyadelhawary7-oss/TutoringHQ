...
- **The sales-lead form hardcodes `governorate: 'cairo'` — CLOSED, 4 August 2026.** Had no field to change it — every lead entered through the CEO dashboard was tagged Cairo regardless of where the center actually was. `sales_leads.governorate` (confirmed live, `text`, already the column `CreateLeadInput` was typed to accept) needed no schema change, just a real selector: added a `<select>` bound to the project's existing `EGYPT_GOVERNORATES` convention (`src/lib/egyptGovernorates.ts`, the same list `checkout` and `admin/demo-requests` already use), defaulting to Cairo, alongside the district free-text field. New `ceo.pipeline.fieldGovernorate` key added to both `messages/en.json` and `messages/ar.json` (real Arabic: "المحافظة", not a placeholder). This entry's own earlier note called it "not a one-line fix" needing a real selector — it wasn't one line, but it also needed no decision and no schema, so it was built rather than logged.
