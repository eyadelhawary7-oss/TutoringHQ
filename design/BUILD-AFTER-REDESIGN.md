...
- **Blast radius, every caller checked by hand rather than assumed.** Now correct off-Cairo (all pass wall-clock strings): `schedule/page.tsx` (8 call sites, `Merged-Center-Groups` §05), `groups/page.tsx` (2), `teacher/GroupSlotsSection.tsx` (5), `teachers/GroupSlotsTab.tsx` (2), `teacher/(portal)/groups/[groupId]/GroupClassesTab.tsx`, `teacher/schedule/SlotActionSheet.tsx`, and `Center-Home` §01's own schedule row. Unaffected because they pass an instant, not a wall clock: `attendance/ScanTab.tsx` (a `Date`), `invoiceTemplates.ts` (`created_at`), `whatsapp/flows/parentNotifications.ts` (a `Date`). Also unaffected: `src/lib/timeFormat.ts` exports a **different, unrelated** `formatTime` and was never involved.
...
Not taken on trust. Fresh grep: `users.preferred_locale` is read in exactly four places (`login/page.tsx`, `api/me`, `api/user/locale`, and signup/provisioning writes) and by **no outbound composition path anywhere** — no WhatsApp send, no notification writer. So D27's claim that per-recipient locale composition is "a pattern used nowhere else today" is correct, independently reproduced.
