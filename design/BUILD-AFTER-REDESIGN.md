...
- ⚠ **CSRF half CLOSED, 4 August 2026, Center-Setup parity pass.** Re-verified live before touching anything: `/api/billing/upgrade`, `downgrade`, `reactivate` and `cancel` had no `validateCSRFRequest` call (confirmed by reading all four route files fresh, not from this entry's own memory) while `/api/billing/withdrawal` and `initiate-payment` already did — exactly as this entry originally found. Fixed by adding the identical `validateCSRFRequest(request, auth.userId)` check already used on `withdrawal`/`cancel`-adjacent routes to all four, and wiring the client (`settings/billing/page.tsx`) to send `getCsrfHeaders(token)` on all four fetches — the same helper the page already imports and uses for `withdrawal`. No schema, no decision: this was the mechanical half of S8. The two misleading-money-figure findings below are **not** touched by this fix and remain open.
- **What (as originally found, money-figure half still open):** every money-moving action on `/settings/billing` — upgrade, downgrade, reactivate, withdrawal, cancel subscription, pack request, invoice pay — is `requireCenterAuth` + owner-role gated, and **none of them call `validateCSRFRequest`**. The one CSRF-checked mutation near this page (`/api/settings/billing`'s PUT/POST) is dead code from this page's own perspective — the billing page never calls it. Same class of gap as **S6**/**S7**, now on the highest-value money surface surveyed yet (subscription upgrade/downgrade/cancel, not just an add-on).
...
## F28 · `Merged-Center-Setup` §05 Support — three of four "HELP" rows and "App version" have no live destination
...
## F29 · `Merged-Center-Setup` §04 Subjects & grades — the design's on/off toggle and the entire Grades concept have no backing schema
