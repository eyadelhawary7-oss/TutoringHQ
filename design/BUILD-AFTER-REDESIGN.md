...
- **CORRECTED for `Center-Attendance` §01, 5 August 2026 — this entry's "blocked **wholesale**" is now STALE for §01, and the correction is not a re-reading of the same evidence, it is new evidence.** **PR #322 (`e7f5dd20`), merged 4 August**, shipped `src/components/verification/VerificationBadge.tsx` + `src/hooks/useVerificationState.ts` + `GET /api/verification/status`: one state machine, failing closed, rendering whichever state is true. §01's actual blocker was never that the screen needs Valify to function — attendance and per-session collection work today without it — it was that the design draws a "Verified" pill in 5 of 5 frames and live had nothing honest to put in that slot. There is now something honest to put there, and `/{locale}/attendance` renders it. **What it reads today is "Verification unavailable", and that is the correct answer, not a shortfall:** `verification_records` and `verification_attempts` are still absent from the live catalog — re-queried directly against `lczmjpnbuhnsislcvzar` on 5 August, `information_schema.tables`, both names, neither present (`payout_requests` does exist and is the pre-existing referral table, not #322's ledger). #322's own body states both migrations are proposals awaiting a manual apply. **§01 therefore moves from "0/2 wholesale-blocked" to a block-level ~4.5/9**, with its remaining gaps owned by D12 (no billing-basis column on `student_groups`, so no "Covered"), D20 (no session to end, so no End-session-and-bill), F20 item 2's own reasoning (Select all is a bulk write on a pipeline with no retry dedup), and the shared-primitive rule (`ActionSheet` has no body slot for the detail sheet's four-fact card). **§02 is NOT corrected and stays blocked wholesale** — and its stronger blocker is now the protected-behaviour rule rather than V1: #322 shipped `POST /api/collection/enable` and `GET /api/collection/status` with **zero UI callers**, and its own body says "No centre-facing payout UI: `Merged-Center-Money.html` is protected and Eyad's." §02 writes, shows money figures, and gates on an entitlement. **The lesson worth keeping: this entry had been "re-confirmed" three times (31 Jul ×2, 4 Aug) and each re-confirmation was true when written; what made the fourth one wrong was a merge to master the day before. Re-confirming a blocker means re-checking the world, not re-reading the entry.**
...
## F44 · `Merged-Center-Setup` §04 Center details draws an Address field that has no column — NEEDS A MIGRATION, stopped rather than written
