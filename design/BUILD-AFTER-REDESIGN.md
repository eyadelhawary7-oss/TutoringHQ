...
## S9 · No CSRF on four CEO/admin mutation routes, one of them a platform-wide kill switch — 3 of 4 CLOSED, 4 August 2026
...
- **Fixed, 4 August 2026 (Merged-CEO parity pass, `claude/parity-ceo-w2`), 3 of the 4:** re-verified live first — still zero `validateCSRFRequest` calls on any of the four, confirming this entry was not stale. `POST /api/ceo/leads`, `PATCH /api/ceo/actions/[id]`, and `PATCH /api/ceo/platform-config` now call `validateCSRFRequest(request, auth.userId)` and reject with 403 when it fails, matching the pattern in `src/lib/csrf.ts` and every other protected admin route. `/ceo/page.tsx`'s `getAuthJsonHeaders()` was updated in the same change to call `getCsrfHeaders()` (the existing shared client helper from `src/lib/csrf-client.ts`, already used by `admin/centers/[id]/centerManagementClient.tsx` and `admin/platform-config/page.tsx`) so both sides landed together as this entry itself required. Grepped first to confirm these three routes have exactly one caller each (`ceo/page.tsx`), so the fix cannot affect any other screen.
- **⚠ DEPLOY PREREQUISITE — `CSRF_SECRET` must be confirmed set wherever this deploys, BEFORE merge.** This was the unaddressed risk in the original write-up: verifying the *callers* proves the client sends the headers, which is necessary but not sufficient. `src/lib/csrf.ts` fails CLOSED — `isCSRFEnabled()` requires a value of **exactly 64 hex characters**, and when it is absent or malformed `validateCSRFRequest()` returns `false` unconditionally, so all three routes above go from working today to returning 403 on every request, *in every environment*. One of them (`PATCH /api/ceo/platform-config`) is the platform-wide `maintenance_mode`/`read_only_mode`/`cron_paused` kill switch, so the failure mode is "the switch you would reach for in an incident is the one that stopped working". Fail-closed is correct and was deliberately **not** weakened to de-risk this change; the mitigation is confirming the secret, not softening the check. Two supports added this pass so the dependency cannot be missed again: `scripts/check-env.ts` now audits `CSRF_SECRET` for presence *and* 64-hex well-formedness against the environment it runs in (the pre-existing parity check could never catch this — the name is in `.env.example`, so it always passed), and `.env.example`'s comment now states the format and the 403 consequence, because the placeholder it ships (`your-key-here`) is itself neither 64 chars nor hex and silently produces the broken state for anyone who copies it verbatim.
- **`PATCH /api/admin/centers/[id]` deliberately left open — this is a territory boundary, not an oversight.** That route is also called by `admin/centers/[id]/centerManagementClient.tsx` (which already sends CSRF headers via the same `getCsrfHeaders()` helper — safe) **and by `admin/centers/page.tsx`'s blacklist-toggle modal, which does not** (grepped: zero matches for `getCsrfHeaders`/`X-CSRF-Token` in that file). Adding the server-side check now would 403 that page's blacklist mutation. Both `admin/centers/*` pages sit under Admin-Accounts/Admin-Platform, one of the six files explicitly called out as under a separate, concurrent Valify workflow this pass must not touch. Fixing the 4th route is one line (`validateCSRFRequest` + the same client-side header addition on the one un-instrumented call site) but belongs to whichever pass owns those files, not this one.
- **Touches:** auth, and money/account-state (the platform-config and center-mutation routes).
- **Blocked by:** nothing technical for the 3 closed here. The 4th needs coordination with the Admin-Accounts/Admin-Platform pass, not Eyad's product judgment — flagging that distinction so it isn't miscategorized as another V/D/X wait.
...
## F30 · `sales_leads.governorate` defaults to `'cairo'` in the database, so the fabricated-Cairo bug survives the form fix
- **What:** `sales_leads.governorate` is `text NOT NULL DEFAULT 'cairo'::text` — verified live 4 August 2026 against `information_schema.columns` on project `lczmjpnbuhnsislcvzar`, not inferred from a migration file. The CEO sales-lead form's hardcoded `governorate: 'cairo'` was fixed this pass with a real `EGYPT_GOVERNORATES` selector, but the *database* still supplies the identical wrong value to any insert that omits the field. The bug was moved one layer down, not removed.
- **Why the form fix does not cover it.** `POST /api/ceo/leads` does `const body = (await parseBodyWithLimit(request, 65536)) as CreateLeadInput` — an **unchecked type assertion, not a runtime schema validation**. `CreateLeadInput` (`src/types/ceo.ts`) does require `governorate` via `Pick<…>`, so TypeScript enforces it on any *typed* caller, but a cast validates nothing at runtime: a request body posted without the field passes straight through to `createLead()`, which does a bare `.insert(input)` (`src/lib/ceo.ts:64-66`) and lets Postgres fill in `'cairo'`. There is no Zod schema on this route to catch it.
- **Severity today: latent, not live.** There is exactly one insert path (`ceo/page.tsx` → `POST /api/ceo/leads` → `createLead`), and after this pass it always sends an explicit governorate. `select count(*) from public.sales_leads` returns **0** — the table is empty, so nothing is currently mis-tagged and there is no backfill to perform. This is logged because the *next* writer of a lead-insert path (an import, a public "talk to us" capture, a partner feed) inherits a silent wrong-default with no type error and no test failure to warn them — the same shape as F13's zero-writer column, inverted: here the value is never absent, it is quietly wrong.
- **Options, none taken here (each needs Eyad):** (a) drop the default and let the `NOT NULL` constraint force every writer to be explicit — cleanest, and safe *specifically because the table is empty*, so there is no existing row to violate it; (b) keep the default but add a Zod schema on `POST /api/ceo/leads` so the API rejects a missing governorate rather than inventing one; (c) accept it and document Cairo as an intentional business default. Not built: (a) is a schema change and this repo applies migrations by hand (working rule 5), and choosing between "reject" and "intentional default" is a product call about how leads get captured, not a code cleanup.
- **Found:** 4 August 2026, by adversarial re-read of this pass's own "CLOSED" claim on the governorate bug — the form was checked, the column default was not.
- **Blocked by:** Eyad's call on (a)/(b)/(c). No technical blocker.
...
- **The sales-lead form hardcodes `governorate: 'cairo'` — NARROWED, NOT CLOSED, 4 August 2026.** (Corrected the same day: this entry first read "CLOSED", which overclaimed. The *form* is fixed; the *mis-tagging path* is not fully shut.) The form had no field to change it — every lead entered through the CEO dashboard was tagged Cairo regardless of where the center actually was. `sales_leads.governorate` (confirmed live, `text`, `NOT NULL`, already the column `CreateLeadInput` was typed to accept) needed no schema change for the UI fix: added a `<select>` bound to the project's existing `EGYPT_GOVERNORATES` convention (`src/lib/egyptGovernorates.ts`, the same list `checkout` and `admin/demo-requests` already use), defaulting to Cairo, alongside the district free-text field. New `ceo.pipeline.fieldGovernorate` key added to both `messages/en.json` and `messages/ar.json` (real Arabic: "المحافظة", not a placeholder).
  - **What remains open — the column default.** `sales_leads.governorate` carries `column_default 'cairo'::text` in the live catalog (verified this pass via `information_schema.columns`, not inferred). Any insert path that omits the field still silently tags Cairo, so the fabricated-data risk is narrowed to the one form that was fixed, not eliminated at the source. Logged as **F30** with the specifics.
  - **What does NOT remain open — the backfill.** An earlier review of this pass listed "nothing backfills leads already mis-tagged Cairo" as an outstanding item. Checked live rather than assumed: `select count(*) from public.sales_leads` returns **0**. The table is empty, so no mis-tagged row exists and there is nothing to backfill. Recording the check itself, since "we should probably backfill" is exactly the kind of plausible-sounding item that gets carried forward for months without anyone counting the rows.
- **§01 Fee-revenue KPI and 6-month revenue chart — re-verified live, 4 August 2026, sharper than previously recorded.** `Merged-CEO` §01 draws a "Revenue, last 6 months" bar chart and a "Fee revenue" KPI (31,494 EGP) distinct from "MRR (subscriptions)" (312,000) — the design's own numbers sum exactly (312,000 + 31,494 = 343,494, the hero total), so this is a real third revenue bucket in the design's model, not a rounding artifact. Two independent problems, not one: **(a) no history deep enough for a 6-month trend** — `mrr_snapshots` (queried live) runs 2026-04-04 to 2026-08-03, 121 daily rows, ~4 months, not 6, and daily where the design wants monthly bars; `/api/ceo/mrr` already exposes this data as a 30-row time series but has zero callers anywhere in `src/app` (dead endpoint, confirmed by grep) — the plumbing for *a* chart exists, just not one with 6 months behind it. **(b) "Fee revenue" as a distinct number does not exist anywhere in the codebase** — confirmed by grep across `src/lib` and `src/app/api` for `fee_revenue`/`feeRevenue`/`processing_fee` aggregation: none. The two real candidates that could plausibly back the label are (i) the flat 20 EGP processing fee snapshotted per-invoice into `invoices.metadata.processing_fee`, which exists per-row but is never summed anywhere (0 invoices in the current month even carry the key, so today's answer would round to zero regardless), and (ii) parent-pack/WhatsApp revenue, which the live dashboard already surfaces separately as `cash.pack_revenue_mtd` (Section F) under a different name. Building either without knowing which one — or a third thing — the design means would mean inventing a number to fill the exact kind of gap this pass exists to catch. **Not attempted.** Needs Eyad: what "Fee revenue" is, and whether a 4-month daily series gets shown truncated/relabeled or waits for more history.
