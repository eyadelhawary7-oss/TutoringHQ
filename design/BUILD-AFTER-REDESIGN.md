...
- **What's actually true, re-verified live 4 August 2026, Center-Orders survey, `information_schema.columns` + full read of every call site:**
  1. **`card_orders` has no `card_style` column at all — confirmed again** (`card_order_carts` has it; `card_orders` does not: 32-column dump of `public.card_orders`, no `card_style` anywhere).
  ...
  3. **Confirmed against live data, not inferred:** `select count(*) from card_orders` → **0**. `select count(*) from card_order_carts` → **0**. No center has ever completed checkout, and no center has even started a cart. This is consistent with (but not solely explained by) the insert bug — `select count(*) filter (where card_orders_enabled) from centers` → **0 of 2** centers currently have the feature flag on, so the broken insert has not yet been hit by a real customer. The bug is 100%-reproducing and would fire on the first center anyone flips the flag for.
...
- **Nothing built here for #1/#2/#4** (all schema-decision-gated, per F26). **#4's downstream symptom that *is* schema-free was fixed this pass** — see F26 item #4 (`card_order_status_transitions.created_at` → `transitioned_at`) — done below.
