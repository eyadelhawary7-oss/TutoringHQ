...
- **Reconfirmed 4 August 2026 (Teacher-Setup structural build, PR #338).** Re-read `JoinCenterCard.tsx` directly: `PROFILE_BASE` is still the literal `'https://tutoringhq.app/teacher/profile'` and the share tab still renders both a copyable link and a QR at `${PROFILE_BASE}/${teacherId}`. Re-globbed the whole app-router tree: still no `teacher/profile/[…]` page anywhere under `src/app/[locale]/`. Unchanged, still a 404, still Eyad's call.
...
- **Found:** 29–31 July 2026 as the un-itemised half of the "0/2" V1 gap; **promoted to its own entry 4 August 2026** (PR #338) because its sibling half — the collect-payments toggle — has since shipped (`e7f5dd20`, PR #322) and the two no longer share a blocker. The toggle needed Valify state; this needs schema.
...
- **Found:** 4 August 2026, structural build of `Merged-Teacher-Setup` (PR #338). Not previously itemised: the coarse 16-item frame scored "group proposals" as one present section, which hid this.
