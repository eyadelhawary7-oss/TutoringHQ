...
## 1.4 Added — Privacy erasure carve-out

Appended to Privacy "Your rights under the PDPL", after the existing *"Some data may be kept where the
law requires it"* sentence. Full text in §3 below.
...
|---|---|
| National ID number | Students, parents, contacts |
| Legal name | Groups, rooms, branches, schedule |
| Amounts | Attendance records |
| Dates | WhatsApp messages and templates |
| Receipt references | Login credentials, PIN hash, sessions |
| | Payout destination (IBAN, wallet) |
| | Tax card number |
...
much, when, and against which receipt. Anything that does not answer one of those four questions is
not in it. Note what is *not* kept — the **payout destination**. We must show that we paid Aly Shady
1,200 EGP on 12/07 against receipt EX-1187. We do not need to keep his bank account to prove it.

## 3.2 Carve-out pattern for the two promises

Both take the same shape as the existing five-year security-records carve-out — a general promise,
then a named exception with its own clock.
...
**Both exceptions name the same five fields and the same clock.** Two different retention rules on one
record is how this gets got wrong later.

## 3.3 What the person is told

Not a legal notice — a plain screen at the moment they ask. **This state is undrawn anywhere in the
105 designs** and needs adding to `Merged-Public-Legal`'s data-rights confirmation.

> **Your data is deleted.** We removed your students, groups, attendance, messages, contact details
> and login.
>
...
**This is not a full erasure and the policy should not call it one.** A record keyed to a National ID,
a legal name and payment amounts remains personal data for the whole retention period. The right
framing is *restriction of processing* — kept, not used — which is a distinct right under Law 151.

**Worth asking Adsero whether the skeleton can be pseudonymised**: hash the ID for storage and hold
the plaintext only in the issued receipt PDFs. Whether an inspector accepts that is their call, not
mine.
