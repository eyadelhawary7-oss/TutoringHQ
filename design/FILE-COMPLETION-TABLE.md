...
| 14 | **Center-Setup** | 9/9 | **~2.9/5 avg across §02–§09, re-verified 4 Aug** (survey #253, 31 Jul; §02/§05 nudged up, §09 corrected down, PR #282; §04 gains a real "Manage branches" link, §03's CSRF half of S8 closed, this pass) — §01 not scored (structural divergence, **D28**), §02 ~2.7/5, §03 ~4/5, §04 ~4.5/5, §05 ~1.7/5 (**F28** added), §06 ~2/5, §07 structural ~4/5 / functional ~1.5/5 (**F19**), §08 ~1/5 (**V6**), §09 ~2.5/5 — Requests tab (counter-offer + offer history) re-confirmed fully built on this pass's read, not re-scored line-by-line | D8, D9, D10, D11, D28, F19, F24, F28, F29, S8 (CSRF half CLOSED), V6 | **This pass (4 Aug):** CSRF added to `/api/billing/{upgrade,downgrade,reactivate,cancel}` + client `getCsrfHeaders` wiring (closes S8's mechanical half, re-verified live before touching, matches `withdrawal`'s existing pattern); real "Manage branches" link + count added to `/settings/center` (sourced from the same `/api/branches` the Branches screen itself reads, not invented); i18n `manageBranches` key added to both locales with real Arabic. Prior pass (#282): Account personal-info card, Support email+Legal links, `passwordIs` i18n fix. | **F19** team invite/activate-deactivate/seat-cap are all broken in production (schema + account-state issues, Eyad's go-ahead needed) · **S8** money-figure half still open (fictitious downgrade "credits earned," dead reactivation tier UI, upgrade preview excludes the processing fee) — CSRF half closed this pass · **F28** (new) Support's Help center/Report a problem/Request a feature/App version have no live destination or data source · **F29** (new) Subjects' on/off toggle and the entire Grades concept have no backing column/table · **D8** seats · **D9** notif prefs (closed) · **D10** scanner prefs · **D11** `/settings/general` is the hub, not a language/region screen · **D28** Onboarding is a structural product divergence, not an unbuilt design · **F24** My Teachers Slots tab is a different feature than drawn (attached-group confirm vs. open marketplace) · §08 → **V6** | no — F19, S8 (money-figure half), D8, D9(closed)/D10/D11/D28 decisions, F28, F29, V6 |
