...
| 5 | **Teacher-Students** | 2/2 | **complete — D15 closed** (re-verified 4 Aug; D15 shipped 3 Aug, PR #310, one day before this survey — the table had not caught up. §01 and §02 read against the design section by section this pass, both structurally complete: search/filter/roster, and a dedicated `/teacher/students/[id]` detail route with contact, balance, attendance, recent classes all sourced from real, live-confirmed columns) | D15-CLOSED | — (nothing left to build; see notes) | **D15-CLOSED** — `Mark collected` and `Send reminder` both ship; the reminder is fail-visible-blocked on Meta template approval (external, not a product decision) with a named on-screen reason, never a fake "sent." Two cosmetic deviations remain and are correctly *not* built, not decisions: the design colours one sample row's group tag brass with no stated rule and no live column to key it off, and phone numbers render in the app's tabular sans (there is no true monospace face wired in `--font-mono`, a global typography call, not a per-file one) | **yes** |
...
| 7 | **Teacher-Groups** | 5/5 | **~2.2/5** (section-level survey, 31 Jul; re-verified live 4 Aug on `claude/parity-teacher-groups-w2`, all four D-codes unchanged — §01/§02 largely intact, §03's review-gate structure absent, §04 missing mark-collected/Vodafone Cash, §05 unbuilt; coarser than rows 2–6's sub-element counts, see notes) | D18, D19, D20, D21 | — (4 Aug pass: re-verified D18-D21 against the live catalog/code with fresh evidence, found one new fact — a dormant, zero-row, zero-reader `group_join_links` table with a `token` column — that doesn't close D21; a D20 fix was prototyped to confirm it's mechanically small, then reverted unshipped per that entry's own pre-existing decision block) | **D18** §03 review gate doesn't exist — every enrollment auto-activates, no schema for school/note fields · **D19** `finish_class_and_bill` never populates commission/net columns · **D20** `SlotActionSheet` recorded phase is read-only (no mark-collected, no Vodafone Cash) · **D21** join link is a full UUID, not the design's 6-char code (a matching `group_join_links.token` column exists but is unused/unwired) · §05 (Class Session Verified — Digital/Cash split, CIB payout) is entirely unbuilt and the design's own caption marks it "draft, pending legal review" — money/legal-adjacent, left untouched like the protected files | no — D18, D19, D20, D21, §05 |
| 8 | **Center-Orders** | 4/4 | **3.5/4** (survey 31 Jul; re-verified 4 Aug, structure unchanged — the new finding is live breakage, not a missing section) | **R8** (built, #231), D7, F18, **F28** (new) | Arabic `ordersEmpty` string fixed + §01 empty-state illustration built (PR #272) · this pass (4 Aug): `card_order_status_transitions.created_at`→`transitioned_at` rename (F26 item 4, free fix) + the repo-wide `,`-as-missing-value bug (F29) fixed in this file's own 5 screens | **F28 (new, corrects F26)** the checkout `card_orders` INSERT itself writes a `card_style` column the table doesn't have — every checkout attempt 500s, unconditionally, plus the centre's own `/orders/[orderId]` page 404s for the same reason (not just the admin PDF route F26 named) — 0 of 2 centers have the feature flag on today, so unhit in production but guaranteed to fail the first center that gets it · **F18** Customize step has no per-field print toggle (schema change) · **D7** notify-me has no destination table | no — F28, F18, D7 |
| 9 | **Center-Insight** | 3/3 | **≈3.5/5** (§01 4.2/5, §02 4.5/5, §03 1.6/5 — parity pass, 4 Aug: §01's collection-rate gauge card and §03's rate-decay step chips built; both other sections re-verified structurally unchanged) | D13-CLOSED, D9-CLOSED, D22, D33, S7-CLOSED | §01 "Collection rate" card (ring gauge + collected/outstanding breakdown, previously-fetched-and-discarded `outstanding_total` now rendered) · §03 commission-structure list restyled to the design's 25%/10%/5% step chips, sourced from the live `COMMISSION_TIERS` constant (this pass) | **D22** §03's KPIs and R6's rate/countdown UI all read `referral_reward_records`, a table with zero live writers, instead of the real cron target `referral_commissions` — Eyad's call on which table is canonical · **S7 now CLOSED** — fixed on master, PR #308, confirmed live 4 Aug (`validateCSRFRequest` is in the route) · **D33** (newly coded this pass; previously only a table-cell mention) — the forecast tile and dashed projection bar need a decided extrapolation method, not just data, since linear day-of-month scaling and a due-date-aware method give different numbers · D13/D9 are **closed** decisions (paid-add-on gate parked, benchmark metric drawing corrected 4-not-5) — correctly not built, not a re-open | no — D22, D33 (D13/D9 are resolved, not gaps; S7 is closed) |
| 10 | **Center-WhatsApp** | 3/3 | **§01 3/5, §02 0/5, §03 0/4** (§01 built #275, 31 Jul; re-surveyed 4 Aug — search/preview/variables re-confirmed live and correct, D4/D5 re-confirmed with fresh live queries, nothing further buildable found; row-level icon + a raw-enum category label were the only two display gaps still open and both closed this pass, no schema/decision touched) | D4, D5 | search + WhatsApp-style preview + variables-used (#275); per-template icon (mirrors the design's row icon, name/category-keyword mapped) + human-readable category label replacing the raw `card_orders`/`MARKETING`/`UTILITY` enum text (this pass, 4 Aug) | **D4** per-template auto-send toggle — schema exists (`center_message_templates.auto_send`), reconfirmed 0 rows / 0 columns of `enabled` or `auto_send` on the table the live route actually reads (`wa_meta_templates`), zero readers, unmade product decision that spends WhatsApp credit unattended · **D5** §02/§03 are a different, already-shipped billing model (monthly per-parent pack `PACK_PRICE_PER_PARENT=12` EGP/parent/month + capped blasts `BLAST_PRICE_PER_PARENT_INCLUSIVE=9.8` EGP, reconfirmed live) vs. the design's one-time credit top-up (200/1,000/5,000 fixed tiers + custom-amount flow) — building the design changes what existing customers are charged | no — D4, D5 |
| 11 | **Center-Groups** | 5/5 | **~4.4/5** (re-verified 4 Aug against PR #313, which shipped between the 1 Aug survey and this pass and closed D31 plus six other blockers — §01 Groups ~0.95, §02 Verified fully blocked unchanged, §03 Rooms ~0.95 on the shared `ActionSheet`, §04 Branches ~0.85 now on `ExpandableRow` with real Switch/Dashboard/Edit/More actions — **D31 closed**, §05 Schedule ~0.9 with day/week both reachable at every width and a real conflict indicator) | D2-CLOSED, D12, D23, D31-CLOSED, D32, F11, **F31** (new, fixed this pass) | this pass: 8 live Arabic-translation bugs fixed in the `groups`/`rooms`/`branches` namespaces actually read by these screens (`t('groupNameRequired')` etc. — see BUILD-AFTER-REDESIGN.md **F31**); #313 (already on master before this pass): the full §01/§03/§04/§05 rebuild that closed D31 | **D12** §02 billing basis, no schema · **D23** §04 add-branch pricing clones the parent's price wholesale — #313 made the add-on notice config-gated (`platform_config['branch_addon.monthly_price_egp']`, absent live) instead of hardcoded, but the pricing decision itself is unchanged and still Eyad's · **D32** the design's simple per-row "Add" promote button is confirmed live and working (`handleAddMember`, stale-waitlist-entry bug fixed) — what's still Eyad's call is only whether to ALSO build the automatic WhatsApp-reply promotion path · **F11** `capacity_cap`/`kind` on `student_groups` re-confirmed live in `information_schema.columns` 4 Aug, still zero references in `src/` — drop-or-document, unchanged | no — D12, D23, D32, F11 |
| 12 | **Center-Students** | 4/4 | **~2.45/4, unchanged in fraction, one more gap closed** (§01 0.8/1, §02 0.75/1, §03 0.05/1, §04 0.85/1 — survey #257 baseline ~54%, re-verified + built 31 Jul PR #277; re-verified again 4 Aug, `claude/parity-center-students-w2` — F23 closed, F12/F14/F15/F22 re-confirmed unchanged on fresh evidence, not from memory) | F12, F14, F15, F22, V1/V6 | ID card tile, tinted balance card, sibling rows, lifetime-paid-since, import inline Fix (31 Jul, #277; closed F22 items 3–4); dashboard deep-link query params (`?filter=unpaid`, `?action=add`) wired to the roster's existing segment/add-modal state (4 Aug, closes F23) | **F22** item 1 branch rollup — corrected 4 Aug: `GET /api/branches` already queries across sibling centers by `organization_id`, so the `branches` count can be genuinely org-wide; the `active` count is not (fetched scoped to the caller's own `center_id`), so the two clauses of "N active · M branches" don't share a scope — needs an RLS-scope decision on whether the `active` half should sum across sibling centers too, not a display fix · item 2 aging/next-due sub-line — `getStudentBalances()` is a running aggregate with no per-invoice "next due" fact to read · **F12** `pending_enrollments` has no origin column (re-confirmed live 4 Aug) · **F14** parent-phone-required copy vs. actually-optional validation — sharpened 4 Aug (the design's own worked example agrees with its copy; live has no self-contradiction either; genuinely a product-behaviour decision, not a bug) · **F15** lifecycle+payment-badge fusion is a design decision · **V1/V6** §03 unconditional "Verified" badge, Valify not live (re-confirmed 4 Aug: §03 is simply unbuilt, not a fabricated badge) | no — F22 (2 open items), F12, F14, F15, V1 |
...
| 14 | **Center-Setup** | 9/9 | **~2.9/5 avg across §02–§09, re-verified 4 Aug** (survey #253, 31 Jul; §02/§05 nudged up, §09 corrected down, PR #282; §04 gains a real "Manage branches" link, 4 of S8's 7 CSRF gaps closed — not all of them, this pass) — §01 not scored (structural divergence, **D28**), §02 ~2.7/5, §03 ~4/5, §04 ~4.5/5, §05 ~1.7/5 (**F32** added), §06 ~2/5, §07 structural ~4/5 / functional ~1.5/5 (**F19**), §08 ~1/5 (**V6**), §09 ~2.5/5 — Requests tab (counter-offer + offer history) re-confirmed fully built on this pass's read, not re-scored line-by-line | D8, D9, D10, D11, D28, F19, F24, F32, F33, S8 (CSRF 5/7 — 2 routes still open), V6 | **This pass (4 Aug):** CSRF added to `/api/billing/{upgrade,downgrade,reactivate,cancel}` + client `getCsrfHeaders` wiring — **partial, not S8's whole CSRF half: `/api/parent-pack/request` and `/api/invoices/[id]/pay` are still unprotected**, and S8's original claim that `withdrawal` had no CSRF was itself wrong (it always had it — see the correction logged on S8); real "Manage branches" link + count added to `/settings/center` (sourced from the same `/api/branches` the Branches screen itself reads, not invented); i18n `manageBranches` key added to both locales with real Arabic. Prior pass (#282): Account personal-info card, Support email+Legal links, `passwordIs` i18n fix. | **F19** team invite/activate-deactivate/seat-cap are all broken in production (schema + account-state issues, Eyad's go-ahead needed) · **S8** still open on two fronts: **CSRF is 5/7, not done** — `/api/parent-pack/request` and `/api/invoices/[id]/pay` still call no `validateCSRFRequest` after this pass — and the money-figure findings are untouched (fictitious downgrade "credits earned," dead reactivation tier UI, upgrade preview excludes the processing fee) · **F32** (new) Support's Help center/Report a problem/Request a feature/App version have no live destination or data source · **F33** (new) Subjects' on/off toggle and the entire Grades concept have no backing column/table · **D8** seats · **D9** notif prefs (closed) · **D10** scanner prefs · **D11** `/settings/general` is the hub, not a language/region screen · **D28** Onboarding is a structural product divergence, not an unbuilt design · **F24** My Teachers Slots tab is a different feature than drawn (attached-group confirm vs. open marketplace) · §08 → **V6** | no — F19, S8 (2 CSRF routes still open + the money figures), D8, D9(closed)/D10/D11/D28 decisions, F32, F33, V6 |
...
| 19 | **Teacher-WhatsApp** | 0/1 | **0/1, correctly blocked** (survey #261, 31 Jul; re-confirmed unchanged, batch-4 sweep, same day; re-verified live against production + `src/` again 4 Aug, no drift, nav confirmed clean with no dead link, per-template requirement matrix added to D6) | D6 | — | **D6** — credit-balance columns are real and marketed but the spend RPC has zero callers, so the number only ever goes up; 0 of 5 drawn templates deliver today, each for a different verified reason (1 platform-wide Meta-approval gap, 3 never submitted, 1 pair that doesn't exist for anyone); no prepaid-pack model to extend from centers | no — D6 |
| 20 | **Public-Legal** | 1/1 | **7/7 states drawn; copy 201/201 design strings live; document text 13/23 sections** (re-survey, 5 Aug — the row below it was stale by two PRs) | X4 | — (nothing left that is not X4) | **X4** legal text from Adsero — the only external blocker with no money/auth attached | no — X4 only |
...
**Row 20 correction, 5 August 2026.** The row read *"chrome/routing ~100%, text 0% — … every
section body is `[This section will be completed upon legal review]`"*. That was false on both
halves, and had been since **PR #311** (`81639a14`, confirmed an ancestor of `origin/master`)
landed the Public-Legal parity build. The literal placeholder string it quoted returns **zero**
matches anywhere in `src/` today. Re-measured from the design file rather than from this row:

- **States: 7/7.** Index, four readers, data-rights form, confirmation — all drawn, both languages.
- **Copy: 201/201.** Every distinct visible string in `Merged-Public-Legal.html`'s twelve frames
  (`.intro`, `.dname`, `.rsec`, `.rp`, `.rli`, `.rtitle`, `.flabel`, `.chk`, `.fnote`, `.dh`,
  `.dt`, `.stx`, `.toc a`, `.callout`, `.askbar`, `.btn`, the `.fin` placeholders) is present on
  the live surface. The four the extraction reported absent are composed at render time
  (`emailLabel`+`emailHint`, `calloutLead`+`calloutBody`), not missing.
- **Document text: 13/23 sections.** The design lists 23 contents entries across the four
  documents and draws prose for 13. The other **10** carry an empty `blocks` array and render one
  explicit "Pending Adsero draft." line. That 13/23 is X4's exact size, and it is now locked by
  `tests/unit/legalCorpusParity.test.ts`, which derives the split from the design file itself so
  neither invented copy nor a quietly deleted section can pass.

The lesson is the one this file's own preamble already states: **a status row is not evidence.**
This one was two PRs behind and a survey that trusted it would have re-done finished work.
...
confirmed the "probably fine" read. `Teacher-Students` came back nearly clean (one gap, D15) and by
4 Aug that gap closed too (PR #310, re-verified the same day this table was corrected) — it is the
first file in this table to reach 100%.
...
D7, its only two gaps, are both schema/decision items, not survey unknowns. Center-Insight (≈3.5/5,
parity pass 4 Aug) had its S7 CSRF gap close on master in the meantime (PR #308, verified by direct
read, not by the PR description) — the remaining shortfall is D22's dead referral table plus D33, a
forecast-methodology decision that existed informally in this table's own text for days before it got
a code and an evidence trail of its own. Two real structural gaps closed this pass without touching
either: §01's collection-rate figure had a real gauge+breakdown card built from data the API was
already computing and discarding, and §03's commission ladder became the design's step-chip visual
sourced from the same live constant Admin-Accounts already uses, so the two screens cannot drift
apart. D13 and D9 turned out to be resolved decisions correctly reflected in what's *not* built, not
open questions. Center-WhatsApp (§01 3/5 after
carry a live production bug underneath its design-structure score: **F28**, found this pass, shows the
...
- **Center-Orders' 3.5/4** — §01 and §02 counted whole (empty-state gap closed via PR #272; no
  design-structure gap on a full re-read). §03 and §04 each carry one confirmed schema/decision gap
  (F18's print toggle, D7's notify destination) and are counted at 0.75 each, per PR #272. The 4 Aug
  re-verification found F28 — a live checkout/order-detail bug underneath §02/§03 — but it does not
  move this number: the design's sections and data model are all correctly represented in the code,
  the code just can't execute because of a missing column. Structure coverage measures IA, not uptime.
