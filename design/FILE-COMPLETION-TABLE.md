...
**8–10 are all surveyed now too.** Center-Orders (3.5/4, PR #272; re-verified 4 Aug) turned out to
carry a live production bug underneath its design-structure score: **F28**, found this pass, shows the
card-order checkout INSERT references a column (`card_orders.card_style`) that was never added to
that table — every checkout attempt fails, and the centre's own order-detail page 404s the same way.
Not yet hit by a real customer (0 of 2 centers have the feature flag on), but it corrects F26, which
had only attributed this column's absence to the admin PDF route. F18 and D7 remain the other two
schema/decision gaps; none of the three are survey unknowns. Center-Insight (≈3.3/5,
