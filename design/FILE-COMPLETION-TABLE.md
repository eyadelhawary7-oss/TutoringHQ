...
| 15 | **Public-Marketing** | 4/4 (§04 built via **R1**, #254 — route-coverage cell was stale, corrected) | **survey #254, 31 Jul; §02/§03 built out further, PR #286, same day** — §01 not scored (structural divergence), §02 improved (teacher-landing comparison table + FAQ added, still partial — 2 of 4 design FAQs answered, `ComparisonTable`/`TrustSignals` sub-components not fully read), §03 strong on core plan pricing, add-ons now **confirmed** mostly fabricated (**D29**, was "unconfirmed" before this pass), §04 (R1) fully built | R1 (built, #254), D29, D30, F25 | shared `MarketingFooter` across all 4 pages, teacher-landing comparison table + FAQ, `/pricing` CTA pair to `/talk-to-us`, a real 299→499 EGP price-mismatch bug fixed (this pass, #286) | **D29** 5 of 6 pricing add-ons have no real backing config, would advertise SKUs that don't exist · **D30** an "8–12 hours saved" claim the design's own header calls "banned by your own rules" is still live · **F25** two parallel WhatsApp-number config sources used inconsistently, one silently drops the CTA · §01 structural divergence (flagship landing-page rearchitecture is a product-scope call) | no — D29, D30, F25, §01's scope question |
