...
| 17 | **CEO** | 2/3 | ⚠ **merge prerequisite: confirm `CSRF_SECRET` is set (64 hex) on Vercel before merging the S9 fix — `src/lib/csrf.ts` fails closed, so an unset secret 403s the platform kill switch.** **§01 structural superset but §01's "Fee revenue"/6-month chart confirmed absent (re-verified live 4 Aug, sharper: no aggregate exists for it anywhere, and `mrr_snapshots` is exactly 4 months of daily rows, not 6 of monthly), §02 real IA divergence (5 live tabs vs. the design's overview+leaderboard), §03 0/2 correctly blocked** (survey #256, 31 Jul; re-confirmed + sharpened, PR #288, 31 Jul; re-verified live again, `claude/parity-ceo-w2`, 4 Aug) | V5, S9 (3/4 closed), F21 (closed), F30 | F21 price-source fixed, dead `legacyPayload` removed, "second CEO dashboard" confirmed dead code (PR #288) · **this pass (4 Aug):** S9 CSRF added to 3 of 4 mutation routes (`ceo/leads`, `ceo/actions/[id]`, `ceo/platform-config`) with the matching client-side header change; sales-lead form's hardcoded `governorate: 'cairo'` replaced with a real `EGYPT_GOVERNORATES` selector (no schema, no decision) — **narrowed, not closed, see F30** | **V5** §03 Centers Benchmark blocked on Valify, schema-catalog-confirmed zero verification columns/constraints on `centers` (re-confirmed live 4 Aug) · **S9 residual** — `PATCH /api/admin/centers/[id]` (the 4th route, used for suspend) deliberately left open: shared with `admin/centers/page.tsx`'s blacklist toggle, which sends no CSRF headers, and that page sits under Admin-Accounts/Admin-Platform, a different pass's territory right now · §01's Fee-revenue KPI/chart needs a product call on what "Fee revenue" even is (no candidate aggregate matches; per-invoice `processing_fee` is real but never summed, parent-pack revenue is already a separate live KPI) plus whether a 4-month daily series ships truncated or waits · §02's 5-tab IA vs. the design's overview+leaderboard is a product-scope question, not a bug | no — V5, S9 residual (cross-territory, not Eyad's), §01/§02 scope questions |
