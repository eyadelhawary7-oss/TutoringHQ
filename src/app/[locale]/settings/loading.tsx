    ...
    <div className="bg-[var(--color-surface-0)] min-h-screen p-4 md:p-6 pb-[calc(56px_+_env(safe-area-inset-bottom,0px))] md:pb-6">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {[...Array(3)].map((_, i) => (
          <div
            key={i}
            className="rounded-xl border border-[var(--color-border-subtle)] p-6 space-y-4"
          >
            <div className="h-5 w-48 bg-[var(--color-surface-2)] animate-pulse rounded-xl" />
            <div className="h-10 w-full bg-[var(--color-surface-2)] animate-pulse rounded-xl" />
            <div className="h-10 w-full bg-[var(--color-surface-2)] animate-pulse rounded-xl" />
            <div className="h-24 w-full bg-[var(--color-surface-2)] animate-pulse rounded-xl" />
          </div>
        ))}
      </div>
    </div>
