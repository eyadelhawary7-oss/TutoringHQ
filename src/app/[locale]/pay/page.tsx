...
export default function CustomerInvoicesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/billing/customer-invoices', { headers: hdr });
  ...
  const onDownload = useCallback(async (invoiceId: string) => {
    setDownloadingId(invoiceId);
    try {
      const hdr = await authHeader();
      if (!hdr) return;
      const res = await fetch(`/api/invoices/${invoiceId}/pdf`, { headers: hdr });
      if (!res.ok) return;
