  ...
  const loadGroups = useCallback(async () => {
    ...
    try {
      const { data } = await dbSelect({
        ...
        select: 'id, name, fee, subject, attendance_mode',
        filters: [
          { column: 'center_id', op: 'eq', value: centerId },
          { column: 'attendance_mode', op: 'eq', value: 'checklist' },
        ],
        order: { column: 'name' },
      });
      const rows = (data || []) as (ChecklistGroup & { attendance_mode?: string })[];
      ...
      {!selectedGroup ? (
        loadingGroups ? (
          ...
          <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-surface-1)] p-8 text-center">
            ...
            <p className="text-sm text-[var(--color-text-secondary)]">{t('noChecklistGroups')}</p>
          </div>
        ) : (
