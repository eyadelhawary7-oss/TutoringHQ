    ...
    return (
      <div className="min-h-[calc(100vh-8rem)] flex items-center justify-center px-4 py-8">
        <div className="max-w-md w-full rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-surface-1)] p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-[var(--color-text-secondary)]">{t('whatsapp.packDisabledDesc')}</p>
          <button
            type="button"
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (res.ok) {
                  toast.success(t('whatsapp.packEnabled'));
            ...
            className="mt-6 inline-flex items-center justify-center gap-2 rounded-lg bg-teal-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-teal-700 disabled:opacity-50"
          >
            {togglingPack ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
            {t('whatsapp.enablePack')}
          </button>
        </div>
      </div>
    );
          ...
          onClick={async () => {
            ...
            try {
              ...
              if (res.ok) {
                ...
                toast.success(t('whatsapp.packDisabled'));
              }
            } finally {
              setTogglingPack(false);
            }
          }}
          className="text-sm font-medium text-slate-400 hover:text-white border border-slate-600 rounded-lg px-3 py-1.5 disabled:opacity-50"
        >
                            ...
                            <td className="px-3 py-2">
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  if (res.ok) {
                                    const data = (await res.json()) as { activeParents?: number; activeCount?: number };
                                    const next =
