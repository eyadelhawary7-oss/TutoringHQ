...
// NOTE: The direct-provision `POST /api/admin/team` entry point was REMOVED. Adding a team
// member now goes through the invite-link → self-service intake → CEO-approval flow:
//   * POST   /api/admin/staff-invites          — CEO mints a single-use invite link
//   * POST   /api/staff-invite/submit          — invited person submits an inert intake
//   * GET    /api/admin/staff-requests         — CEO's pending queue (super_admin only)
//   * PATCH  /api/admin/staff-requests/[id]    — CEO approves (provisions) or declines
// Provisioning (auth identity + set-PIN link) happens only on approval, reusing
// provisionStaffLogin. The GET/PUT/DELETE handlers here (list / edit role / deactivate)
// are unchanged.
