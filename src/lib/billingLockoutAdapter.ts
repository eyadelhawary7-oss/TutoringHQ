...
/**
 * Record a ledger event. Returns TRUE only when THIS call created the row, so a
 * one-shot side effect (a WhatsApp nudge) can be gated on it. A unique-index conflict
 * (Postgres 23505 on billing_lockout_events_oneshot_uq) means a concurrent tick, or a
 * repeated Cairo hour on the fall-back DST edge, already recorded this one-shot for
 * the day, so we return false and the caller must NOT send again. Any other error is
 * logged and also returns false, so a transient failure never causes a double-send.
 */
async function recordEvent(
  ...
  const { error } = await admin.from('billing_lockout_events').insert({
    center_id: centerId,
    cairo_day: cairoDay,
  ...
  if ((error as { code?: string }).code !== '23505') {
    console.error('[billingLockoutAdapter] recordEvent failed', eventType, error);
  }
  return false;
}
...
export function createSupabaseLockoutAdapter(
  ...
  return {
    ...
    async applyInvoiceNudge(center): Promise<void> {
      ...
      // sends the accompanying "due today" nudge. Gate the send on a NEWLY recorded
      // ledger row so two concurrent ticks (or a repeated Cairo hour) cannot
      // double-send: the second insert conflicts, recordEvent returns false, and no
      // second WhatsApp goes out.
      const created = await recordEvent(admin, center.centerId, center.billingDayCairo, 'invoice_nudge');
      if (created && center.ownerPhone) {
        await sendTemplateMessage(center.centerId, center.ownerPhone, TEMPLATE_FIRST_NUDGE);
      }
    },

    async applyReminder2(center): Promise<void> {
      // Gate on a newly recorded row (see applyInvoiceNudge) so a repeated tick never
      // re-sends the second reminder.
      const created = await recordEvent(admin, center.centerId, center.billingDayCairo, 'reminder2');
      if (created && center.ownerPhone) {
    ...
    async applyLock(center): Promise<void> {
      ...
      // the "locked" nudge, gated on a newly recorded row so the repeated fall-back
      // DST hour (or a manual re-run) cannot re-send it. No bulk status write, no
      // amount change.
      const created = await recordEvent(admin, center.centerId, center.billingDayCairo, 'lock');
      if (created && center.ownerPhone) {
