      ...
      <div className="mx-auto max-w-4xl text-center">
        ...
        >
          {tiles.map(({ key, Icon }) => (
            <div
              key={key}
              className="rounded-xl border border-slate-800/40 bg-slate-900/60 px-[18px] py-4 text-start"
            >
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-teal-500/10">
                <Icon className="h-[18px] w-[18px] text-teal-400" />
              </div>
              <p className="mt-3 text-sm font-medium text-white">{t(`${key}.title` as 'tile1.title')}</p>
