...
export function CancelOrderModal({
  ...
  const warningKey =
    status === 'pending_payment'
      ? 'warn.pending_payment'
      : status === 'vendor_assigned'
        ? 'warn.vendor_assigned'
        : 'warn.paid';
  ...
  const canSubmit = confirmText === 'CANCEL' && otherOk && !submitting;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-4">
      ...
      >
        ...
        <p className="text-sm text-[var(--color-text-secondary)]">{tcancel(warningKey)}</p>
