...
### Frame coverage, measured 6 August 2026 and re-measured after seeding

Frames enumerated from the HTML by counting `class="phone"`, which is the only element present
exactly once per frame in all 25 files. Each frame was then checked against the data in Test Center
333. **26 of 32 can now be produced.**

| File | Frames | Exercisable | Blocked |
|---|---|---|---|
| `Merged-Center-Groups` | 18 | **13** | 5 |
| `Merged-Center-Students` | 14 | **13** | 1 |

Six frames were unblocked by a second seeding pass, each verified rather than assumed: 3
`pending_enrollments` with deliberately different row shapes (one without a parent phone, one without
a note), **exactly one** same-room slot collision (Chemistry A 16:00–18:00 against Biology A
17:00–19:00, Room 2, day 0), and 2 waitlisted students at positions 1 and 2.

**The collision doubles as a live test of the room-clash guard.** `schedule/page.tsx:487` and `:490`
should flag that one pair and leave the other eight slots alone. **More than one flagged means the
guard is wrong; none flagged means the fix never landed.**

**Still blocked, 6 frames, and the two reasons need different answers.**

**Schema, 3 frames.** All of `Merged-Center-Groups` §03 Branches. No branch table exists. A migration
proposal, to be written after the diff completes rather than before.

**Blocked by design, 3 frames.** `Center-Groups` §01 Empty, `Center-Groups` §02 Rooms empty state,
`Center-Students` §01 Empty. Producing them needs a second centre with an owner and an empty roster.
**That vehicle has never existed** — the deleted Test Owner Center had zero users and could not be
logged into, so it could not have rendered them either. A second tenant is not being created for
three frames. **If a signup-flow test is ever built, these come free with it**, because a
just-created centre is exactly an owner with an empty roster.

Report these three as blocked-by-design with that reason, not as failures.
