...
--
-- BUT THE BRANCH HAS A HARD DEPENDENCY THAT THIS FILE DOES NOT SATISFY.
-- Do not deploy the sweeper until claude/payout-2-2-withdrawal-race has landed
-- AND its migration (process_withdrawal_request) has been applied by hand.
-- On origin/master the admin approve/reject path releases the reservation
-- BEFORE flipping the request status, leaving a span in which the row is still
-- 'pending' and a sweeper claim succeeds — two cancel_reservation_atomic calls
-- for one request, against a credit_reserved counter shared with
-- combined_payment_sessions. See the DEPENDENCY header in
-- src/lib/withdrawalReservationSweep.ts.
--
-- VERSION PREFIX: this file was 20260804160000 and was renamed to
-- 20260804180000 to resolve a three-way collision on that timestamp
-- (2.2 keeps ...160000, 2.1 keeps ...170000, this is 2.5, S10 takes ...190000).
--
...
--
-- db/schema.snapshot IS regenerated by this branch, and doing so applied
-- nothing to production. The snapshot is rebuilt OFFLINE by
-- scripts/schema/rebuild.sh, which replays the committed migration files into a
-- throwaway local PostgreSQL cluster and dumps the catalog from there. It never
-- connects to lczmjpnbuhnsislcvzar. The snapshot therefore describes the schema
-- as the migration files DEFINE it, which is deliberately ahead of production
-- until Eyad applies this file by hand.
-- =============================================================================
