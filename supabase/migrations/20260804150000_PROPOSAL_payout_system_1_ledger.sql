...
--
-- ****************************************************************************
-- * APPLY ORDER: THIS IS THE SECOND OF TWO. Apply                             *
-- *   20260804140000_verification_records_proposal.sql                        *
-- * FIRST. It creates verification_records / verification_attempts, which are  *
-- * what src/lib/collectionPayout/verificationGate.ts reads to decide whether  *
-- * a principal may be paid out. Applying this ledger alone gives you a payout *
-- * system whose money gate can only ever refuse — which is safe, but is not   *
-- * the state you are trying to reach.                                        *
-- *                                                                            *
-- * These two files were both numbered 20260804140000 on their original        *
-- * branches. `supabase_migrations.schema_migrations.version` is a PRIMARY KEY,*
-- * so only one of them could ever have been recorded; the second would have   *
-- * looked applied while never having run. This one was renumbered to          *
-- * 20260804150000. A third proposal, which put verification columns directly  *
-- * on `centers` and `teacher_profiles`, shared the same version and is        *
-- * DELETED rather than renumbered — its facts now live in the tables the      *
-- * 140000 file creates.                                                       *
-- *                                                                            *
-- * Verified live 4 August 2026: the highest version in                        *
-- * supabase_migrations.schema_migrations is 20260804094631, so both 140000    *
-- * and 150000 are free.                                                       *
-- ****************************************************************************
--
-- ============================================================================
-- PRECONDITIONS — re-queried LIVE on 4 August 2026 against project
-- lczmjpnbuhnsislcvzar, read-only. Every line below was re-run by the
-- consolidation agent, not carried over: the original of this header said the
-- public schema had 137 base tables and `centers` had 108 columns. Both were
-- wrong (142 and 128). The absences they were cited to support are all real,
-- which is exactly how the wrong counts survived three audits.
-- ============================================================================
--
--   base tables in public ................. 142
--     select count(*) from information_schema.tables
--     where table_schema='public' and table_type='BASE TABLE';
--
--   ledger_accounts ....................... ABSENT  (not among those 142)
...
--
--   centers ............................... 128 columns. credit_balance and
...
--     Deliberately writes its own row rather than a new column on `centers`, so
--     this proposal does not alter the 128-column centres table for a feature
--     that is still gated on identity verification. Same reasoning as the
--     identity migration's part 6(a): `centers` has one SELECT policy,
--     `centers_select_own USING (id = get_auth_center_id())`, RLS cannot
--     withhold a column, and four `select('*')` call sites would pick up
